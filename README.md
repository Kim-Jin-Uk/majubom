...
| 경로 | 주기 | 왜 그 주기인가 | 명세 |
|---|---|---|---|
| `POST /api/cron/cleanup-unverified` | 일 1회 | 7일 경과 미검증 신청 삭제 — 급하지 않다 | (없음) |
| `POST /api/cron/expire-requests` | 5분 | `REQUESTED` 가 슬롯을 묶고 있다. 늦으면 팔 수 있는 자리가 잠긴다 | C2 |
| `POST /api/cron/auto-no-show` | 일 1회 04:00 | 종료 후 `autoNoShowAfterHours`(기본 24h)를 넘긴 `CONFIRMED` 정리 | C3 |
| `POST /api/cron/expire-swaps` | 매시 | 근무 교대 요청 72시간 무응답 만료 — 72시간짜리 시한에 분 단위는 의미가 없다 | (없음) |

넷 다 조건부 UPDATE 라 재실행이 무해하고, 한 번에 최대 500건씩 처리한다.
만료 배치가 멈추면 콘솔 요약의 "승인 대기" 가 계속 늘어난다 — 그게 신호다.
**코드 주석에는 아직 `C1`·`C8` 표기가 남아 있다** — 아래 설계의 ③ 단계에서 지운다.

#### 앞으로: 번호를 버리고 **틱 셋**으로 합친다 (설계 확정, 미구현)

**C 번호는 명세 문서의 것이다. 코드는 이름으로 부른다.** 지금 코드의 `C1`(미검증 정리)은 명세의 `C1`(리마인더)이
아니고 `C8`(교대 만료)은 명세 표(C1~C7)에 없는 번호다 — 코드가 만든 배치 둘이 명세 표에 없어서 임의로 붙인 번호다.
번호를 맞추려면 정본 문서를 고쳐야 하고, 고쳐도 배치를 하나 더 만들면 같은 일이 반복된다. 그래서 **번호를 버린다.**
명세를 읽을 때만 C 번호를 쓰고, 대응은 아래 표 한 장으로 둔다.

**잡은 셋으로 합친다.** Cloud Scheduler 무료 한도가 job 3개인데 지금 넷이고, 명세대로 다 만들면 일곱에 백업까지다.
비용(초과 1개당 월 $0.10)보다 **운영 면적**이 문제다 — 잡마다 시크릿·재시도·알림 설정이 따로 있어서, 하나가 조용히
멈춰도 아무도 모른다. 주기가 다른 것만 나누면 셋으로 충분하다.

| 틱 | 경로 | 주기 | 하는 일 | 명세 번호 |
|---|---|---|---|---|
| 짧은 틱 | `POST /api/cron/tick` | 5분 | 승인 대기 만료 | C2 |
| 매시 틱 | `POST /api/cron/hourly` | 정각 | 예약 리마인더(#14) · 교대 만료 | C1 · (없음) |
| 야간 틱 | `POST /api/cron/nightly` | 03:00 KST | 사용량 집계(#67) · 자동 노쇼 · 미검증 신청 정리 · 개인정보 마스킹 · 채팅 정리 | C4 · C3 · (없음) · C5 · C6 |

**백업(C7)은 합치지 않는다.** 앱이 고장 났을 때도 돌아야 하는 유일한 배치다 — 앱 프로세스 안에 넣으면
앱이 죽는 순간 백업도 같이 죽는다. `pg_dump` 를 도는 별도 Cloud Run job 으로 간다.

야간 틱의 규약 다섯:

1. **순서는 집계 → 상태 변경 → 삭제·마스킹.** 마스킹이 먼저 돌면 그날 집계가 이미 지워진 값을 센다.
   (사용량 집계 → 자동 노쇼 → 미검증 정리 → 마스킹 → 채팅 정리)
2. **한 작업의 실패가 다음을 막지 않는다.** 각각 잡아서 넘어가고 응답에 작업별 처리 건수·실패를 담되,
   하나라도 실패하면 **500** 을 낸다. Scheduler 재시도와 알림이 거기 걸려 있어서, 200 으로 삼키면 며칠씩 조용히 안 돈다.
3. **한 번에 끝내려고 타임아웃을 늘리지 않는다.** Cloud Run 300초다. 작업마다 상한(500건)을 두고 남으면
   응답에 `pending` 을 담아 **다음 날이 아니라 다음 매시 틱**이 이어받는다. 합쳐서 생긴 유일한 새 위험이
   "야간 틱 하나가 길어져 뒤가 밀리는 것" 이라, 장치는 여기만 둔다.
4. **전부 멱등.** 조건부 UPDATE 라 중간에 상태가 바뀐 건은 409 로 건너뛰고, 리마인더는 발송 이력에 없는 건만 본다.
5. **시각은 03:00 하나로 당긴다.** 명세가 03:00·04:00·05:00·05:30 으로 나눈 이유는 부하 분산인데,
   한국 시간 03시대는 트래픽이 가장 낮고 작업 총량이 수백~수천 행이다. 무거워지면 그때 야간 틱을 둘로 가른다 —
   그래도 잡은 하나만 는다.

인증·관측은 그대로다: `X-Cron-Secret`, 틀리면 404, 작업별 처리 건수를 구조화 로그로, 연속 2회 실패는 Sentry.

**#67(사용량 집계)은 야간 틱의 한 단계**다. `usage_counters`(사업장 × 날짜)에 그날 생성 예약 수·발송 알림 수·
활성 매니저 수·상품 수·이미지 저장량·마지막 활동 시각을 적는다. **소급 집계**가 핵심 규약이다 —
`max(date)` 부터 어제까지를 메우므로 하루 걸러도 다음 실행이 따라잡는다. 날짜 경계는 KST(관리자 지표와 같은 규칙).
관리자 대시보드(#69)는 이 테이블을 읽지 않는다 — 그건 사업장별 *사용량*이고 대시보드가 보는 것은 전역 *품질*이다.

옮기는 순서: **① 틱 셋을 만들고 기존 넷의 본체를 그 안에서 부른다**(기존 경로는 남겨 둔다 — Scheduler 를 먼저 바꿔야 한다)
→ **② Scheduler 잡을 셋으로 다시 만든다** → **③ 기존 네 경로와 주석의 `C1`·`C8` 표기를 지운다** → **④ 사용량 집계를 야간 틱에 넣는다(#67)**.
