...
**`main` 룰셋은 다시 `Active` 다 (9/11 확인).** 회사 이메일을 지우느라 껐던 것을 되돌렸다 —
삭제·force-push 금지, 선형 히스토리, PR 필수, 필수 체크 `verify`·`firestore-rules`·`secret-scan`.
그 사이 #181·#182·#183·#178 넷은 보호 없이 들어갔으므로, 문제가 보이면 그 구간을 먼저 의심한다.
`claude-review` 는 일부러 필수에서 뺐다(게이트가 아니다 — `claude-review.yml` 주석).
