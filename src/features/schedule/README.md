...
**요청 폼을 감출 때는 이유를 말한다.** 교대를 요청하려면 셋이 필요하다 — 읽기 전용이 아닐 것, 내 계정에 담당자 자원이
연결돼 있을 것, **상대가 한 명이라도 있을 것**(계정이 연결되고 활성인 다른 STAFF). 조건을 조용히 걸러 폼만 감추면
"페이지는 열리는데 뭘 해야 할지 모르겠다" 가 된다 — 로컬 테스트에서 실제로 그랬다.
특히 상대가 0명인 경우가 조용했다: 본인 담당 자원은 있으니 기존 안내("담당자 계정만…")도 뜨지 않고 폼만 사라졌다.
판정은 `swap-rules.ts` 의 `swapEligibility` 하나이고, 화면 위에는 요청 → 수락 → 승인 세 단계를 적어 둔다 —
**승인이 사장님 몫**이라는 것은 그 문장이 아니면 화면에서 드러나지 않는다.
