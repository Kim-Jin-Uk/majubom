...
휴무는 발생일(반복이면 앞으로 365일 — maxAdvanceDays 상한)의 예약 — 부분 휴무는 시간 겹침만. 전날 저녁에 시작해 자정을 넘긴 예약도 그 날짜의 충돌로 본다. 충돌이 있으면 409 로 목록을 돌려주고 OWNER 만 `confirmConflicts`/`keepReservations` 로
