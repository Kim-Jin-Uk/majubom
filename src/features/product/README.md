...
**날짜 범위 판정도 상품 시간을 본다.** `dateInRange`/`stillRunning` 이 사업장 영업시간만 보면,
20:00~02:00 클래스를 **00:30 에 조회할 때 그 영업일이 "어제" 로 걸러져 예약이 아예 안 된다** —
이 기능의 대표 예시에서 바로 터지는 자리다. 판정의 입력을 고르는 것은 `effectiveOpeningHours` 하나다.
위젯 달력의 하한(`firstDate`)도 **어느 상품이든 열려 있는 가장 이른 날**로 넓혔다 —
하한을 넓게 잡으면 그날 슬롯이 없는 상품은 달력이 비활성으로 그리지만, 반대(하한이 좁음)는 손님이 칸을 누를 수조차 없다.
