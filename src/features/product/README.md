...
**상품마다 시간을 따로 정할 수 있다 (9/14 결정).** 기본은 **"영업시간과 동일"** 이고, 그 상태에서는
`products.opening_hours` 가 **빈 배열**이다 — 값을 저장하지 않는 것 자체가 연동이다. 그래서 사업장 영업시간을
바꾸면 이 상품도 같이 바뀐다(스냅샷이 아니다). 복사해 두면 그 연결이 끊긴다.

**상품 시간이 있으면 사업장 영업시간을 대신한다 — 교집합이 아니다.** 교집합으로 두면
"영업 10–19, 클래스 20–22" 같은 상품을 아예 만들 수 없다(빈 구간이 된다). 맞추고 싶으면 토글을 켜면 된다 —
어느 쪽을 원하는지 사장님이 고르고 엔진은 고른 것을 따른다. 판정은 `schedule/resolve.ts` 의 `productWindows` 하나다.

**사업장 영업시간이 없으면 토글을 켤 수 없고, 상품 시간이 필수다.** 켜 봐야 따라갈 값이 없고, 둘 다 비면
하루 전체가 열려 아무 시각이나 예약된다. 판정은 **서버가** 한다(`checkAgainstBusiness`) — 클라이언트가 보낸 값을 믿지 않는다.
토글을 끄면 사업장 영업시간을 **시작점으로** 채워 준다. 빈 표에서 일곱 요일을 다시 찍게 두지 않는다.

요일 편집기는 사업장 설정과 **같은 컴포넌트**다(`business/ui/HoursEditor.tsx`) — 두 곳에 따로 쓰면
휴게 규칙·자정 넘김·빠른 설정이 한쪽만 고쳐진다.
