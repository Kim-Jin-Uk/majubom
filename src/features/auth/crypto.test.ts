...
describe("password (scrypt)", () => {
  ...
  it("낮은 파라미터 해시는 재해시 대상", () => {
    expect(passwordNeedsRehash("scrypt$16384$8$1$a$b")).toBe(true);
    expect(passwordNeedsRehash("scrypt$131072$8$1$a$b")).toBe(false);
  });
});
...
describe("server proof", () => {
  it("같은 (purpose, subject, secret) 만 통과", () => {
    const p = serverProof("mfa", "sid-1", "secret".repeat(6));
    expect(verifyServerProof(p, "mfa", "sid-1", "secret".repeat(6))).toBe(true);
    expect(verifyServerProof(p, "mfa", "sid-2", "secret".repeat(6))).toBe(false);
    expect(verifyServerProof(p, "bind", "sid-1", "secret".repeat(6))).toBe(false);
    expect(verifyServerProof(undefined, "mfa", "sid-1", "secret".repeat(6))).toBe(false);
  });
});
