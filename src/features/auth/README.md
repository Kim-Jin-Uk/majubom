...
**콘솔 "매 요청 재확인" 의 범위.** `/console` `/admin` `/api/console` `/api/admin` 은 accessExp 와 무관하게 매 요청
`users.status · business_members.status · businesses.status(+emailVerifiedAt) · sessions.revoked_at` 을 본다 (인덱스 조회 2~3회).
스냅샷이 달라지면 JWT 를 그 자리에서 다시 쓴다 — 권한 변경도 15분을 기다리지 않는다. 고객 경로는 15분 스냅샷을 믿는다.
...
- `mfaProof`: `HMAC(AUTH_SECRET 파생키, "mfa:" + sid)` 가 맞을 때만 `mfa=ok` (`crypto.serverProof`)
...
**비밀번호는 scrypt.** Node 내장이라 네이티브 빌드가 없다(CI·App Hosting 빌드팩에서 깨질 여지 없음). N=2^17·r=8·p=1·64B,
포맷에 파라미터를 넣어 나중에 올려도 기존 해시가 검증되고 로그인 시 재해시된다.

**로그인 실패 백오프는 감사 로그가 카운터.** `LOGIN_FAIL`(target = 이메일 해시) 15분 창 5회부터 1s→32s. 실패는 어차피
전부 감사 대상이고 인스턴스가 늘어도 DB 하나라 일관된다. TOTP 오입력도 같은 장치(키 `totp:<uid>`).
