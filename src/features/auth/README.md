...
**로컬·CI 에서는 TOTP 를 건너뛸 수 있다** (`AUTH_DEV_SKIP_TOTP`, `dev-flags.ts`). `/admin` 은 첫 진입에서
TOTP 등록을 강제하는데 인증 앱을 붙일 수 없는 곳에서는 관리자 화면을 아예 열 수 없다. 그렇다고 코드에서 검사를 빼면
운영자 계정이 비밀번호 하나로 열리므로 **환경으로만** 연다.

잠금이 두 겹이다 — 판단이 `dev-flags.ts` 한 곳이라 호출부마다 `NODE_ENV` 를 다시 보다 한 군데를 빠뜨릴 일이 없고,
`serverEnv` 스키마가 **production + 이 플래그** 조합을 거부해 실수로 배포 설정에 넣으면 **부팅이 실패한다**.
값도 정확히 `"true"` 만 받는다(`"1"`·`"yes"` 는 안 켜진다) — 오타 하나가 2단계를 끄면 안 된다. `dev-flags.test.ts`.
