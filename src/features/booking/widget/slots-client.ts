 ...
 */
export function windowFor(monthStart: string, today: string, maxAdvanceDays: number, daysInMonth: number): { from: string; to: string } | null {
  const monthEnd = addDays(monthStart, daysInMonth - 1);
  const last = addDays(today, maxAdvanceDays);
