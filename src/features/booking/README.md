...
**빈자리를 묻는 것은 승인뿐이다.** 거절은 그 자리에서 끝나는 전이라 옮긴 자원을 점유하지 않는다 —
거기에도 검사를 걸면 "마침 내가 그 시각에 다른 확정 건이 있다" 는 이유로 **남의 예약을 거절하지도 못하게** 된다.
승인은 반대다: 넘겨받은 뒤에도 그 자리를 계속 쓰므로 겹치면 409 `SLOT_TAKEN` 이어야 한다.
