...
**가동률** = 확정·완료 예약이 **운영시간 안에서** 차지한 분 ÷ 운영 가능한 분. 버퍼가 개점 전·폐점 후로 나간 몫은 세지 않는다 —
...
격자는 그 기간의 운영시간과 예약을 다 담는 범위로만 그린다(빈 새벽을 접는다). 자정을 넘긴 예약은 `endMin > 1440` 으로 시작 날짜 컬럼에 이어 그린다 —
