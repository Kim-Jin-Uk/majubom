...
지금 있는 것은 **슬롯 조회(010) · 생성(020) · 상태 전이(030·040·060) · 변경(050) · 콘솔 목록·상세(080)** 이다.
**워크인(070)은 서버만 있고 화면이 없다** (아래). FR-BOOK 에서 남은 것은 알림·예약 카드다.
...
결정이 아직 안 된 것은 루트 `LATER.md` — L-31(합산 잔여의 의미) · L-33(가동률의 버퍼) · L-34(주 캘린더의 자원 컬럼) · L-35(FIXED 브레이크와 콘솔 지표).
L-06(슬롯 계산 가정 A1~A12)은 해소됐다 — 명세를 고치는 쪽으로 정리했고 docs 저장소 커밋만 사람 몫으로 남았다.
...
**1인 동시 예약 한도는 사업장 × 상품 단위로, 앞으로의 예약만 센다.** 정책값은 사업장 것이지만 세는 범위는 상품까지 좁힌다 —
사업장 단위로 세면 "젤네일 3건" 을 잡은 고객이 그 매장의 페디큐어를 **아예 예약할 수 없다**. 막으려는 것은 한 상품의 자리를
한 사람이 쓸어 가는 것이고, 상품이 다르면 다른 자원·다른 격자다. 지난 예약은 상태 전이(COMPLETED·EXPIRED)가 늦어질 수 있는데
그게 고객을 영구히 막으면 안 된다. check-then-act 라 고객 락 안에서 센다.

**자동 확정은 "언제 사람이 있다" 를 누가 말했는지로 정한다 (9/14).** 아무도 말하지 않았으면 하루 전체가 열려 있다는
뜻이라 `autoConfirm` 을 무시하고 `REQUESTED` 로 받는다 — 매장이 한 건씩 보고 확정해야 한다.
보는 것은 자원 종류마다 다르다: **담당자는 근무표**가 그 선언이라, 근무표가 없으면 **영업시간이 있어도** 제약 없음이다
(영업시간은 "가게가 열려 있다" 지 "이 사람이 출근한다" 가 아니다). **룸·공용은 사람이 없으므로** 영업시간이 곧 이용 가능 시간이다.
판정은 순수 함수 `isUnconstrained` 하나이고 `create-status.test.ts` 가 그 경계를 고정한다.
...
**담당이 아닌 매니저도 콘솔 전이를 할 수 있다** (L-32 결정, 표의 `anyManager`). 오판 교정(OWNER 전용)만 빼고 여섯이다.
기본값은 "본인 담당 건만" 이라, 새 전이를 추가한 날 남의 예약이 조용히 열리지 않는다.

**승인·거절은 담당까지 넘어온다** (`takeOver`). 그건 매장이 손님에게 하는 **약속**이라, 약속한 사람과 그날 서는
사람이 달라지면 손님은 A 의 이름으로 확정 메일을 받고 매장에는 B 가 있다. 대신 승인한다는 것은 **내가 맡는다**는 뜻이다.
완료·노쇼·취소는 이미 끝난 일의 기록이라 담당을 옮기지 않는다.

이관은 전이와 **같은 트랜잭션**이다 — 옮길 자원이 없으면(담당자 자원 미연결 → 409 `NO_OWN_RESOURCE`) 전이도 함께 실패한다.
승인만 되고 담당이 그대로 남는 상태를 만들지 않는다.

**이관은 상태를 바꾼 뒤에 한다.** 순서가 반대면 아직 `REQUESTED` 인 예약을 옮기게 되는데, DB 의 `no_overlap`
EXCLUDE 술어가 `status in (REQUESTED, CONFIRMED)` 라 내 자리가 그 시각에 차 있으면 **거절조차 23P01 로 막힌다** —
앱에서 빈자리 검사를 건너뛰어도 소용없다(CI 가 잡았다). 거절로 먼저 넘어가면 그 술어에서 빠지므로 옮길 수 있다.

**빈자리를 묻는 것은 승인뿐이다.** 거절은 끝난 예약이라 옮긴 자원을 점유하지 않는다 — 거기에도 검사를 걸면
"마침 내가 그 시각에 다른 확정 건이 있다" 는 이유로 **남의 예약을 거절하지도 못하게** 된다.
승인은 반대다: 넘겨받은 뒤에도 그 자리를 계속 쓰므로 EXCLUDE 와 `validateExisting` 이 둘 다 겹침을 막는다.
자원 검사(상품 연결·활성·빈자리)는 사장님의 담당 변경과 **같은 함수**를 쓴다(`moveReservationResource`) —
경로마다 잣대가 다르면 한쪽으로 이중 예약이 샌다. 이관은 `reservation_logs` 에 `from_resource_id`/`to_resource_id` 로
남아 상세 이력에 "담당 변경" 으로 보인다.

화면은 남의 담당 건이면 먼저 묻는다 — 승인·거절은 "담당이 나에게 넘어옵니다" 까지 말한다. 담당자(사람) 자원에만
존칭을 붙인다(룸에 "「대회의실」님" 은 어색하다). **열려 있어도 보여야 처리한다** —
`viewAllReservations` 가 없는 매니저에게 동료 건은 여전히 404 다.
...
**워크인(070)은 서버만 있고 화면이 없다 (9/11 결정).** `POST /api/console/reservations/walk-in` 과 `createWalkIn()`,
DB 회귀 3건까지 다 있는데 콘솔 어디에도 버튼이 없다 — 부르는 클라이언트 코드가 0줄이다.

만들지 않기로 한 이유: 워크인의 값어치는 "현장 손님을 기록한다" 가 아니라 **달력을 진실하게 유지한다**는 데 있는데,
그 효용은 실제 매장이 돌아가야 생긴다. 1기(W1–W15)에는 사업장이 없고, 그때까지는 사장님이 그 시간에 개인 차단(`BLOCK`)을
걸면 슬롯이 닫혀 이중 예약은 막힌다(손님 정보가 안 남을 뿐). **실제 매장을 받은 뒤**(베타 W17 무렵) 사장님이 현장 손님을
어떻게 처리하는지 보고 폼을 정한다. 서버는 그대로 두므로 그때 화면만 붙이면 된다.
